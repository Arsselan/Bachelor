...
nodes = np.linspace(grid.left+extra*0, grid.right-extra*0, ansatz.nDof())
...
def postProcess(animationSpeed=4):
    ...
    ax.plot([left+extra, left+extra], [-0.1, 2], '--', label='left boundary')
    ax.plot([right-extra, right-extra], [-0.1, 2], '--', label='right boundary')
    ...
    animation.save("immersed_bar_bad.mp4", writer=writervideo)

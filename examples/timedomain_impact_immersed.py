...
# solve
penaltyFactor = 1e4
times, u, fullU, evalU, iMat = fem1d.runCentralDifferenceMethodWeakContactImmersedLowMemory(study, dt, nt, u0, u1, evalNodes, penaltyFactor)

# save
if config.stabilize > 0:
    title = config.ansatzType + " s%e" % config.stabilize + " e%e" % config.extra + " n%d" % config.n + " p%d" % config.p + " " + config.mass + " dt%e" % dt + " pen%e" % penaltyFactor
else:
    title = config.ansatzType + " e%e" % config.extra + " n%d" % config.n + " p%d" % config.p + " " + config.mass + " dt%e" % dt + " pen%e" % penaltyFactor

fileBaseName = fem1d.getFileBaseNameAndCreateDir("results/timedomain_impact_immersed/", title.replace(' ', '_'))

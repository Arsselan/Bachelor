...
void evaluate(const BSpline &bspline, 
              const double coord, 
              const int p, 
              std::vector<double> &result);
              
std::vector<double> evaluate(const BSpline &bspline, const double coord) {
    std::size_t n = bspline.knots.size() - bspline.p - 1;
    std::vector<double> result(n);
    evaluate(bspline, coord, bspline.p, result);
    return result;
}
...
void evaluate(const BSpline &bspline, 
    ...
    std::size_t n = knots.size() - p - 1;
        
    if(p==0) {
        for(std::size_t i=0; i<n; i++) {
            if(knots[i] <= coord && coord < knots[i+1]) {
                result[i] = 1;
            } else {
                result[i] = 0;
            }
    ...
    std::vector<double> lowerOrder(n + 1);
    evaluate(bspline, coord, p-1, lowerOrder);
    ...
    for(std::size_t i=0; i<n; i++) {
        ...
        result[i] = 0;
        if(denom1!=0) {
            result[i] += (coord - knots[i]) / (denom1) * lowerOrder[i];
        } 
        if(denom2!=0) {
            result[i] += (knots[i+p+1] - coord) / (denom2) * lowerOrder[i+1];
        }
    }
}
...
void evaluateBSplineBases(const std::size_t i,
                          const double u,
                          const std::size_t p,
                          const size_t n,
